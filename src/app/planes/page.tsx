...
export default function PlanesMunicipiosPage() {
    ...
    try {
      ...
      reader.onload = async (evt) => {
        ...
        const res = await fetch('/api/importar-global', {
          ...
          body: JSON.stringify({ mode: 'planes', data })
        });
  ...
  return (
    <div className="container" style={{ padding: 0 }}>
      ...
      {loading ? <p>Cargando municipios...</p> : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
          {filteredMunicipios.map((m, idx) => (
            <Link key={m.id} href={`/planes/${m.id}`} className="glass-panel" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '1.5rem', textDecoration: 'none', color: 'inherit', position: 'relative', backgroundColor: 'rgba(239, 68, 68, 1.0)' }}>
              <div style={{ position: 'absolute', top: '10px', left: '15px', fontSize: '0.8rem', fontWeight: 600, color: '#000000' }}>
                {String(idx + 1).padStart(2, '0')}
              </div>
              <h3 style={{ fontSize: '1.25rem', color: 'var(--success-color)', margin: '1.5rem 0' }}>{m.nombre}</h3>
      ...
      <Modal isOpen={isImportModalOpen} onClose={() => { setIsImportModalOpen(false); setImportResult(null); }} title="Importación Global de Planes">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <p>Sube tu Excel maestro exclusivo de <b>Planes Pedagógicos</b>. El sistema creará todo automáticamente <b>sin afectar de ninguna forma las Actas de Conformación</b>.</p>
          ...
          {importResult && (
            <div style={{ marginTop: '1rem', padding: '1rem', background: '#dcfce7', borderRadius: '8px', color: '#166534' }}>
              ...
              <ul>
                ...
                <li><b>Planes generados: {importResult.registros}</b></li>
