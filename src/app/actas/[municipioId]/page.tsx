...
export default function ActasInstitucionesPage(props: RouteParams) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
          {filteredInstituciones.map((i, idx) => (
            <Link key={i.id} href={`/actas/${municipioId}/${i.id}`} className="glass-panel" style={{ display: 'block', textDecoration: 'none', color: 'inherit', position: 'relative' }}>
              ...
              <div style={{ marginTop: '0.75rem', fontSize: '0.8rem', color: i._count?.actas > 0 ? '#22c55e' : '#94a3b8', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>{i._count?.actas > 0 ? '✔ Tiene actas registradas' : 'Sin actas aún'}</span>
