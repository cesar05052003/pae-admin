...
export default function EstadisticasActas() {
  const [totales, setTotales] = useState<{ municipios: number; instituciones: number; con: number; sin: number; cobertura: number } | null>(null);
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/estadisticas-modulo?modo=actas').then(r => r.json()),
      fetch('/api/poblacion-indigena/estadisticas').then(r => r.json()),
    ])
      .then(([actas, pi]) => {
        // Combinar totales
        ...
        // Fusionar municipiosData por nombre
        const mapa = new Map<string, MunicipioData>();
        for (const m of (actas.municipiosData ?? []) as MunicipioData[]) {
          mapa.set(m.nombre, { nombre: m.nombre, con: m.con, sin: m.sin });
        }
        for (const m of (pi.municipiosDataCombined ?? []) as MunicipioData[]) {
          const prev = mapa.get(m.nombre);
          if (prev) {
            mapa.set(m.nombre, { nombre: m.nombre, con: prev.con + m.con, sin: prev.sin + m.sin });
          } else {
            mapa.set(m.nombre, { nombre: m.nombre, con: m.con, sin: m.sin });
          }
        }
        setDataChart(Array.from(mapa.values()));

        // Fusionar porTipo
        const tipoMapa = new Map<string, PorTipoItem>();
        for (const t of (actas.porTipo ?? []) as PorTipoItem[]) {
          tipoMapa.set(t.tipo, { ...t });
        }
        for (const t of (pi.porTipo ?? []) as PorTipoItem[]) {
          const prev = tipoMapa.get(t.tipo);
          if (prev) {
            const total  = prev.total + t.total;
            const conCae = prev.conCae + t.conCae;
            tipoMapa.set(t.tipo, {
              tipo: t.tipo,
              label: t.label,
              total,
        ...
        setPorTipo(Array.from(tipoMapa.values()));
